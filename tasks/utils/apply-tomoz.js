...
task('apply-tomoz', 'Apply tomoz token trc21.')
  ...
  .setAction(async function (taskArgs, hre) {
    ...
    if (!eventApplys.length) {
      ...
      if (minCap.gt(amount)) { // minCap > amount
        console.error(`[Error] Apply TomoZ min cap is ${hre.ethers.utils.formatEther(minCap)} ${RPCs[hre.network.config.chainId].nativeCurrency.symbol}`)
        return
      }
      // get issuer in contract
      const erc21Issuer = new hre.ethers.Contract(
        taskArgs.address,
        abiERC21Issuer,
        await hre.ethers.getSigner(),
      )
