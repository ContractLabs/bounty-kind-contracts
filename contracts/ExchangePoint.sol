...
contract TokenProvider is Ownable {
    ...
    function unsetTokens(address[] memory _tokens) public onlyOwner {
        for (uint256 i = 0; i < _tokens.length; i++) {
            if (tokenMap[_tokens[i]].existed) {
                uint256 indexRemove = tokenMap[_tokens[i]].index;
                tokenList[indexRemove] = tokenList[tokenList.length - 1];
                tokenMap[tokenList[indexRemove]].index = indexRemove;
                tokenList.pop();
...
contract ExchangePoint is TokenProvider, Withdrawable {
    ...
    function depositToken(address token, uint256 _amount)
        public
        isValidToken(token)
    {
        IERC20(token).safeTransferFrom(msg.sender, address(this), _amount);
        emit DepositToken(msg.sender, token, _amount);
    }
    ...
    function withdrawToken(
        ...
        address token,
        uint256 _amount
    ) public onlyManager isValidToken(token) {
        require(
            IERC20(token).balanceOf(address(this)) >= _amount,
            "Withdrawable: Insufficent balance to withdraw"
        );
        IERC20(token).transfer(receiver, _amount);
        emit WithdrawToken(receiver, token, _amount);
    }
}